...
def transaction_amount(transactions):
    ...
    for transaction in transactions:
        logger.info("Идет процесс фильтрации валюты в транзакциях")
        if (
            transaction.get("operationAmount").get("currency").get("code") == "USD"
            or transaction.get("operationAmount").get("currency").get("code") == "EUR"
        ):
            url = (
                f"https://api.apilayer.com/exchangerates_data/convert?to="
                f"{code_RUB}&from="
                f"{transaction.get('operationAmount').get('currency').get('code')}&amount="
